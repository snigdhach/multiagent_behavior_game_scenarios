...
homeFlag = [-1.6; -1];   %%Team base location information. To be considered valid, home base must be on the opposite side of the arena.
targetFlag = [1.2;0];
startPoint = [-1.4;-0.8];
endPoint = targetFlag;
proximityRange = 0.01;
xUpBound = 1.6;          %%Arena boundary info.
xLowBound = -1.6;
yUpBound = 1;
yLowBound = -1;
                            %% Depending on the both teams' flag location, random trajectory generation differs.
...
elseif(endPoint(1) < 0 && startPoint(2) > 0)
    ...
    map = 999;  %%Invalid Map!
    disp("Invalid Map, terminate and update team bases location.");
    return;
end
